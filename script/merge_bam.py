...
class BAMMerger:
    ...
    def parallel_sort_and_index_bamfiles(bamfiles, output_dir, num_processes):
        ...
        pool.starmap(BAMMerger.sort_and_index_bamfile, [(bamfile, output_dir) for bamfile in bamfiles])
        pool.close()
        pool.join()
    ...
    @staticmethod
    def split_gff_to_bed(gff_file,chrom_lengths,interval,output_dir):
        #get gene region
        gene_regions = []
    ...
    @staticmethod
    def calculate_coverage(bam_file,all_region_list):
    ...
    @staticmethod
    def process_bam_file(bam,all_region_list):
        coverage = BAMMerger.calculate_coverage(bam,all_region_list)
        print("collect "+ bam + " depth ifomation  finished")
        return bam, coverage

    @staticmethod
    def store_bam_region_coverage_parallel(bam_files,all_region_list):
        now_time = time.time()
        ...
        with multiprocessing.Pool() as pool:
            results = [pool.apply_async(BAMMerger.process_bam_file, (bam,all_region_list)) for bam in bam_files]
    ...
    @staticmethod
    def merge_regions_to_bam(bam_files,coverage_dicts,output_region,all_region_list,output_dir,max_intron_length):
...
def get_bam_file(input_items, min_files=2):
    bam_file_paths = []
    ...
    # 检查是否有足够数量的BAM文件
    if len(bam_file_paths) < min_files:
        print(f"错误: 至少需要 {min_files} 个BAM文件才能合并，但只找到了 {len(bam_file_paths)} 个", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    bam_file_paths=get_bam_file(args.input_folder_or_files,2)
    ...
    merge.sort_and_index_merged_bam(os.path.join(args.output_dir, "merged.bam"), 
        os.path.join(args.output_dir,"merged_sorted.bam"),os.
        path.join(args.output_dir,"merged_sorted.bam.bai"))
